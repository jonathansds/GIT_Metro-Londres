...
public class TrajetoController {
	...
	public Trajeto menorCaminho(final Long estacaoOrigem, final Long estacaoDestino, final Integer linhaOrigem){
		...
		if(!origStation.getLinhasOrigem().contains(linhaOrigem)){
			validator.add(new ValidationMessage("A estação "+origStation.getNome()+" não pertence à linha "+linhaOrigem+".", "Linha Errada"));
		}
		
		validator.onErrorRedirectTo(this).trajeto();
		
		return definirTrajeto(estacaoOrigem, estacaoDestino, linhaOrigem);
	}
	
	@Post
	@Consumes(MediaType.MULTIPART_FORM_DATA)
	@Produces({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON})
	@Path("/trajeto/retornarXML")
	public void menorCaminhoXML(final Long estacaoOrigem, final Long estacaoDestino, final Integer linhaOrigem){
		Station origStation = metroLondres.getStations().get(estacaoOrigem);
		...
		Trajeto trajeto = definirTrajeto(estacaoOrigem, estacaoDestino, linhaOrigem);
		result.use(Results.xml()).from(trajeto).include("estacaoOrigem", "estacaoDestino", "menorCaminho").serialize();
	}
	 ...
	 * @param linhaOrigem
	 * @return menor trajeto
	 */
	private Trajeto definirTrajeto(final Long estacaoOrigem, final Long estacaoDestino, final Integer linhaOrigem){
		Station origStation = metroLondres.getStations().get(estacaoOrigem);
