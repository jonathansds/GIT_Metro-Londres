...
public class DadosController{
	...
	public void importar(final UploadedFile arquivo, final String tabela){
		if(tabela == null || (!tabela.trim().equalsIgnoreCase(Constantes.TABELA_ESTACOES) && 
			...
			validator.add(new ValidationMessage("Não existe uma tabela com o nome indicado!", "nome"));
		}
		
		if(arquivo == null || (!arquivo.getFileName().endsWith(".csv") && !arquivo.getFileName().endsWith(".CSV"))){
			validator.add(new ValidationMessage("Arquivo inválido!", "arquivo"));
		}
		...
		try{
			...
			if(sucesso){
				...
				validator.add(new ValidationMessage("Nenhum valor válido encontrado para ser persistido.", "arquivo"));
			}
